 ...
 * Backup.csv which is handled by DataStorage
 *
 * DESCRIPTION:
 *
...
public class UserApp {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //detect if the for loop is being used in testdriver
        if (args.length > 0) {
            transReader = new UIinput(args[0]);
        } else {
            transReader = new UIinput("");
        }
        ...
        while (transReader.readTransData()) {
            ...
            switch (transaction[0]) {
                case "S":
                    ...
                    numTransactions++;
                    dStorage.selectByCountryCode(transaction[1]);
                    break;
                case "A":
                    numTransactions++;
                    dStorage.showAll();
